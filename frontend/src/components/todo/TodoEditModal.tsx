...
interface TodoEditModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (todoId: string, todoData: UpdateTodoRequest) => void;
  todo: Todo | null;
  loading?: boolean;
}

const TodoEditModal = ({ isOpen, onClose, onSave, todo, loading = false }: TodoEditModalProps) => {
  ...
  useEffect(() => {
    if (todo) {
      setFormData({
        ...
        startDate: todo.startDate.split('T')[0], // Format to YYYY-MM-DD
        dueDate: todo.dueDate.split('T')[0],   // Format to YYYY-MM-DD
        status: todo.status,
      });
    } else {
      // Reset form when todo is null
      setFormData({
        title: '',
        description: '',
        startDate: '',
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
        <div className="p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium text-gray-900">할일 수정</h3>
            <button
              onClick={onClose}
